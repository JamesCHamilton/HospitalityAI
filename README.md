...
If judges ask for ideas on what to input, use these "Stress Test" scenarios to showcase the AI's depth:

| Scenario | Name | Insurance | Clinical Context | Expected AI Behavior |
| :--- | :--- | :--- | :--- | :--- |
| **High Urgency** | Sarah Jenkins | United PPO Platinum | "58yo female with acute chest pain radiating to the neck. History of hypertension. Requires immediate cardio consult." | **Match**: Top Cardiologists. **Adjudication**: `AUTO-APPROVED` with high priority score (90+). |
| **Step Therapy** | Marcus Thorne | Aetna EPO Gold | "Patient requesting total knee replacement for chronic pain. Clinical notes do not mention any prior physical therapy." | **Match**: Orthopedic Surgeons. **Adjudication**: `MANUAL_REVIEW` (Reason: Step therapy required before surgery). |
| **Network Check** | Elena Rodriguez | NY State Medicaid | "Gestational diabetes management. Needs specialist who participates in NY Managed Care." | **Match**: Filtered for Medicaid-participating providers only. **Adjudication**: `AUTO-APPROVED` (In-Network verified). |
| **Safety Logic** | David Wu | BlueCross HMO Silver | "Routine follow-up for stable arrhythmia." | **Safety**: If matched with a provider having a data discrepancy, the status is forced to `MANUAL_REVIEW` for human audit. |

---

## 📈 Demo Instructions
1. **Intake**: Go to the Patient Portal, fill in "John Doe," and attach a referral PDF.
2. **Match**: Select the top match generated by the AI.
3. **Observe**: Open the **Insurance Dashboard** in a separate tab to see the claim appear and adjudicate in real-time via Supabase.
4. **Review**: Check the **Provider Inbox** to see the generated FHIR bundle and e-Fax referral.
